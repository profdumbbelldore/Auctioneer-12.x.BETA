...
--	Stubby.RegisterEventHook("ZONE_CHANGED", "Enchantrix", onEvent)			-- not used right now
	...
	if TooltipDataProcessor and TooltipDataProcessor.AddTooltipPostCall then
		TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, function(tooltip, data)
			if data and data.hyperlink and Enchantrix.Tooltip and Enchantrix.Tooltip.hookItemTooltip then
				-- We pass the link directly to the existing Enchantrix hook
				Enchantrix.Tooltip.hookItemTooltip(tooltip, nil, data.hyperlink, data.quality, 1)
			end
		end)
	else
		-- Legacy fallback
...
-- BfA: data order is screwed up here!
		...
		spellNameIsNIL = NIL;
if (event == "UNIT_SPELLCAST_SUCCEEDED" and player == "player") then
