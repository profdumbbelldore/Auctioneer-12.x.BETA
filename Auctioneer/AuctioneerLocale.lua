...
local locale = Auctioneer.Locale
...
local listCallbacks = {}
local listLocales = {}
-- Parse AuctioneerLocalizations and extract locales - these are our valid locales
for loc, _ in pairs(AuctioneerLocalizations) do
	tinsert(listLocales, loc)
end

-- Auctioneer.Locale.Translate(stringKey [, locale])
...
function locale.Translate(stringKey, locale)
	if not isActive then
		error("AuctioneerLocale: Translate called before active", 2)
	end
...
-- Auctioneer.Locale.ValidateLocale(proposedlocale)
...
function locale.ValidateLocale(proposedlocale)
	if not defaultLocale then
		...
		error("AuctioneerLocale: ValidateLocale called before active", 2)
	end
...
-- Auctioneer.Locale.SetLocale(proposedlocale)
...
function locale.SetLocale(proposedlocale)
	if not isActive then
		error("AuctioneerLocale: SetLocale called before active", 2)
	end

	local config = AuctioneerConfig
...
-- Auctioneer.Locale.GetLocaleList([usetable])
...
function locale.GetLocaleList(usetable)
	if not isActive then
		error("AuctioneerLocale: GetLocaleList called before active", 2)
	end
...
function locale.SetCallbackOnActive(callback)
	if type(callback) == "function" then
		...
		error("Usage: Auctioneer.Locale.SetCallbackOnActive(callback); callback must be a function", 2)
	end
end
...
function locale.OnLoad()
	...
	if not found then
		-- Client locale was not found in AuctioneerLocalizations!
